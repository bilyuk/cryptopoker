...
function assertVerifiedBuyIn(room: RoomRecord, playerId: string): void {
  if (!room.buyIns.some((buyIn) => buyIn.playerId === playerId && (buyIn.status === "escrow-funded" || buyIn.status === "in-play"))) {
...
function verifiedStack(room: RoomRecord, playerId: string): number {
  return room.buyIns
    .filter((buyIn) => buyIn.playerId === playerId && (buyIn.status === "escrow-funded" || buyIn.status === "in-play"))
