...
describe("realtime Room and Player events", () => {
  it("authenticates Socket.IO with the session cookie, authorizes Room subscriptions, emits Room events, and targets Seat Offers privately", async () => {
    ...
    try {
      ...
      await hostVerify(server, firstCookie, roomId, 100);
      ...
      await hostVerify(server, secondCookie, roomId, 120);
      await hostVerify(server, thirdCookie, roomId, 140);
...
async function hostVerify(
  server: Parameters<typeof request>[0],
  playerCookie: string,
  roomId: string,
  amount: number,
): Promise<void> {
  await request(server).post("/buy-ins").set("Cookie", playerCookie).send({ roomId, amount }).expect(201);
}
