...
export class TableSeating {
  claimSeat(player: PlayerDto, room: RoomRecord, seatNumber: number): CommandResult<RoomDto> {
  ...
  leaveSeat(player: PlayerDto, room: RoomRecord): CommandResult<RoomDto> {
    ...
    const events = offerNextSeat(room, seat.seatNumber);
    return commandResult(toRoomDto(room), [{ type: "seat.updated", roomId: room.id }, ...events]);
  }

  joinWaitlist(player: PlayerDto, room: RoomRecord): CommandResult<RoomDto> {
    assertVerifiedBuyIn(room, player.id);
    if (room.seats.some((seat) => seat.playerId === player.id)) {
  ...
  acceptSeatOffer(player: PlayerDto, room: RoomRecord, offer: SeatOfferDto): CommandResult<RoomDto> {
  ...
  declineSeatOffer(player: PlayerDto, room: RoomRecord, offer: SeatOfferDto): CommandResult<SeatOfferDto> {
    if (offer.playerId !== player.id) throw new ForbiddenException({ code: "SEAT_OFFER_NOT_FOR_PLAYER", message: "Seat Offer belongs to another Player." });
    offer.status = "declined";
    removeFromWaitlist(room, offer.playerId);
    const events = offerNextSeat(room, offer.seatNumber);
    return commandResult({ ...offer }, [{ type: "seatOffer.updated", roomId: room.id, playerId: offer.playerId, seatOfferId: offer.id }, ...events]);
  }

  expireSeatOffer(room: RoomRecord, offer: SeatOfferDto): CommandResult<SeatOfferDto> {
